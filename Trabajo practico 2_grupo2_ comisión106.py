...
def comprobacionHora(msj):
        f = True
        while f:
            try:
                hora= input(f"Ingrese {msj} (HH:MM):")
                hs, mn = map (int, hora.split(':'))
                hora = datetime.time(hs,mn)
                f = False
            except ValueError :
...
def comprobacionLen(a,b):
    while len(a) > b or len(a) == 0 or a == " ":
        if len(a) > b :
            a = input(f"La cantidad maxima de caracteres es {b}, por favor reintente: ").strip()
        elif len(a) == 0 :
            a = input("El espacio no puede estar vacío, por favor reintente: ").strip()
            limpiarConsola()
            
             
def pedirNoVacio(msj):
    a = input(f"Ingrese {msj}: ")     
    while (a.strip()==""):
...
def buscarVuelos():
    ...
    # Anchos de columnas (sin ESTADO)
    column_widths = [6, 20, 20, 20, 12, 8, 12]
    ...
    for i, vuelo in enumerate(vuelos):
        ...
        if vuelo[0] != "" and vuelo[6] == "A":
            # Excluir el campo ESTADO (índice 6)
            fila = vuelo[:6] + [f"${precioVuelos[i]:,.2f}"]
            print(format_row(fila, column_widths))

    print("\nTotal de vuelos:", sum(1 for v in vuelos if v[0] != "" and v[6] == "A"))
#Buscar asientos
...
#Se fija cuantas aerolineas tiene cada código de país, devuelve un array con la cantidad de aerolíneas de ARG, CHI, BRA respectivamente
    ...
    while salida != "N":
        while flag and i < 5:
            if aereolineas[0][i] == "":
                codigoPais = str("A")
                salida = str("X")
                aereolineas[0][i] = pedirNoVacio("el nombre de su aereolinea")
...
#funcion de buscar vuelos, pero mostrando el estado de cada vuelo
def buscarVuelosCeo():
    # Encabezados (incluyendo ESTADO)
    headers = ["CÓDIGO", "AEROLÍNEA", "ORIGEN", "DESTINO", "FECHA", "HORA", "ESTADO", "PRECIO"]

    # Anchos de columnas (incluyendo ESTADO)
    column_widths = [6, 20, 20, 20, 12, 8, 8, 12]
    ...
    for i, vuelo in enumerate(vuelos):
        # Solo mostrar vuelos activos
        if vuelo[0] != "" :
            # Incluye el campo ESTADO (índice 6)
            fila = vuelo[:7] + [f"${precioVuelos[i]:,.2f}"]
            print(format_row(fila, column_widths))

    print("\nTotal de vuelos:", sum(1 for v in vuelos if v[0] != ""))
    ...
    while contArray < 20 and flag1 :
        ...
        if contArray < len(vuelos):
            ...
            while flag: #aseguramos que la aereolinea cargada en el vuelo sea una anteriormente aceptada por el sistema
                limpiarConsola()
                vuelos[contArray][1] = pedirNoVacio("nombre de su aerolinea")
                i = 0
                ...
                if i == 5:
                    ...
                    flag = False
            vuelos[contArray][2] = pedirNoVacio("origen de su aerolinea")
            limpiarConsola()
            vuelos[contArray][3] = pedirNoVacio("destino de su aerolinea")
            limpiarConsola()
            vuelos[contArray][4] = pedirFecha("Ingrese la fecha de salida: ")
            limpiarConsola()
            vuelos[contArray][5] = comprobacionHora("Hora de salida del vuelo: ")
            limpiarConsola()
            print("Ingrese el precio: ")
            precioVuelos[contArray] = validarFloat()
            ...
            if vuelos[i][0] == codVuelo and vuelos[i][6]  == "A":
                while opc == "S":
                    ...
                    match opcion:
                        case "Nombre":
                            limpiarConsola()
                            vuelos[i][1] = pedirNoVacio("nuevo nombre de su aerolinea")
                            
                        case "Origen":
                            limpiarConsola()
                            vuelos[i][2] = pedirNoVacio("nuevo origen de su aerolinea")

                        case "Destino":
                            limpiarConsola()
                            vuelos[i][3] = pedirNoVacio("nuevo destino de su aerolinea")
                        case "Fecha":
                            limpiarConsola()
                        ...
                        case "Hora":
                            limpiarConsola()
                            vuelos[i][5] = comprobacionHora("hora de salida del vuelo")

                        case "Precio":
                            limpiarConsola()
                            precioVuelos[i] = validarFloat()

                    opc = input("Desea modificar algun otro campo? S/N: ")
                    while opc != "S" and opc != "N":
                        limpiarConsola()
                        opc = input("Opcion invalida, ingrese nuevamente: ")


            elif vuelos[i][6]  == "B":
                x = input("Desea darlo de alta? S/N: ")
                while x != "S" and x != "N":
                ...
                if x == "S":
                    ...
                    limpiarConsola()
                    print("No se ha podido modificar el vuelo.")
            else:
                i += 1
    if i == 20:
        limpiarConsola()
